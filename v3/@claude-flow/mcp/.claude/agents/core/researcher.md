...
hooks:
  pre: |
    ...
    # 1. Learn from past similar research tasks (ReasoningBank + HNSW ~1.9x-4.7x (measured))
...
- **HNSW Indexing**: ~1.9x-4.7x (measured) knowledge retrieval
- **Flash Attention**: Flash Attention speedup (unverified) for large document processing
...
```typescript
// 1. Search for similar past research (~1.9x-4.7x (measured) with HNSW)
...
console.log(`Search time: ${relevantDocs.searchTimeMs}ms (HNSW: ~1.9x-4.7x (measured))`);
