...
hooks:
  pre: |
    ...
    # 1. Learn from past test failures (ReasoningBank + HNSW ~1.9x-4.7x (measured))
...
- **HNSW Indexing**: ~1.9x-4.7x (measured) test pattern search
- **Flash Attention**: Flash Attention speedup (unverified) for test generation
...
```typescript
// 1. Learn from past test failures (~1.9x-4.7x (measured) with HNSW)
...
console.log(`Search time: ${similarTestCases.searchTimeMs}ms (HNSW: ~1.9x-4.7x (measured))`);
...
10. **Flash Attention**: Generate tests faster (Flash Attention speedup (unverified))
