/**
 * Conformance Kit — runtime types, SimulatedRuntime & MemoryClerkCell
 *
 * The trace/run-result/runtime/cell interfaces, the SimulatedRuntime
 * (fail-closed in-memory platform), and the MemoryClerkCell golden
 * implementation. Extracted verbatim from conformance-kit.ts (lines
 * 37-654) during campaign-2 wave 1 (W207). conformance-kit.ts stays the
 * barrel ('export *'), so the package index.ts re-export blocks resolve
 * byte-identically.
 */
