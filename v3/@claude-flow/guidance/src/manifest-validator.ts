...
// The validation constants + the AgentCellManifest / golden-trace interfaces
// were extracted into ./manifest-validator-types.ts during the P3.37 god-file
// decomposition (W158). Import the runtime enums/limits + type aliases the
// classes use, and re-export every public type so deep importers (and the
// package barrel + ruvbot-integration's inline import()s) resolve unchanged.
import {
  ...
  KNOWN_TOOLS,
  TRACE_LEVELS,
} from './manifest-validator-types.js';
import type {
  DataSensitivity,
  WriteMode,
  AuthorityScope,
