 ...
 */
export async function generateConfigToml(options: ConfigTomlOptions = {}): Promise<string> {
  const {
    model = 'gpt-5.3-codex',
  ...
  lines.push('');
  lines.push('# Approval policy: untrusted | on-failure | on-request | never');
  lines.push(`approval_policy = "${approvalPolicy}"`);
  lines.push('');
  lines.push('# Sandbox mode: read-only | workspace-write | danger-full-access');
  lines.push(`sandbox_mode = "${sandboxMode}"`);
  lines.push('');
  lines.push('# Web search: disabled | cached | live');
