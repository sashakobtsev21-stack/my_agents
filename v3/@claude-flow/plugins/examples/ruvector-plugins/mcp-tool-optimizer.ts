...
interface ILoRAEngine {
  createAdapter(category: string, rank: number): Promise<{ id: string }>;
...
async function createLoRAEngine(): Promise<ILoRAEngine> {
  try {
    const engine = new LoRAEngine({ defaultRank: 8 });
...
class FallbackVectorDB implements IVectorDB {
  ...
  insert(vector: Float32Array, id: string, metadata: Record<string, unknown> = {}): string { this.vectors.set(id, { vector, metadata }); return id; }
  search(query: Float32Array, k: number): Array<{ id: string; score: number; metadata?: Record<string, unknown> }> {
    const results: Array<{ id: string; score: number; metadata?: Record<string, unknown> }> = [];
    for (const [id, entry] of this.vectors) {
      let dot = 0, normA = 0, normB = 0;
      for (let i = 0; i < query.length; i++) { dot += query[i] * entry.vector[i]; normA += query[i] * query[i]; normB += entry.vector[i] * entry.vector[i]; }
...
class FallbackLoRAEngine implements ILoRAEngine {
  private nextId = 1;
  async createAdapter(category: string): Promise<{ id: string }> { return { id: `adapter-${this.nextId++}` }; }
  async updateAdapter(): Promise<void> {}
  computeGradient(input: Float32Array, target: Float32Array): Float32Array {
    const g = new Float32Array(input.length);
    for (let i = 0; i < input.length; i++) g[i] = (input[i] - (target[i] || 0)) * 0.01;
    return g;
  }
}
