/**
 * Diff Classifier — risk assessment & MCP-tool exports
 *
 * The risk types and the file/overall risk, classification, reviewer
 * and sync-analysis helpers. Extracted verbatim from diff-classifier.ts
 * (lines 307-791) during campaign-2 wave 60 (W266). diff-classifier.ts
 * stays the barrel (type-only back-import, W208/W234 shape).
 */

import { execFileSync, execFile } from 'node:child_process';
import { promisify } from 'node:util';
import type { DiffClassification, FileDiff } from './diff-classifier.js';
import { DiffClassifier } from './diff-classifier.js';
