/**
 * MCP tool definitions for the intelligence stats family (4 tools):
 *   - hooks_intelligence_stats      (RuVector stats fan-out — SONA / EWC++ /
 *                                    MoE / Flash / LoRA / ruvllm /
 *                                    contrastive / training / graph DB —
 *                                    with memory-store fallback when the
 *                                    neural backends aren't loaded)
 *   - hooks_intelligence_learn      (force immediate SONA cycle with
 *                                    EWC++ consolidation)
 *   - hooks_intelligence_attention  (FlashAttention diagnostic surface)
 *   - hooks_intelligence_unified-stats  (#2245 → ADR-075: one honest call
 *                                        across the 4 historical stat
 *                                        sources with consistency block
 *                                        flagging drift between them)
 *
 * Extracted from hooks-tools.ts (W52, P3.2 cut #22 — biggest single cut).
 * Imports are routed through the helper modules extracted earlier (memory
 * store + neural loaders) so the parent file's import block can shrink
 * to just MCPTool + the re-export glue.
 */
