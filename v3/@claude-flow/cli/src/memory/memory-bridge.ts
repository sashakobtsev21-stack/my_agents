...
// Pure scoring helpers (BM25 keyword relevance + cosine similarity)
// moved to ./memory-bridge/scoring.ts (W64, P3.4 cut #1). Consumed by
// bridgeSearchEntries (BM25 hybrid re-rank) + bridgeSearchHNSW (cosine).
import { bm25Score, computeTermDocFreqs, cosineSim } from './memory-bridge/scoring.js';

// Shared bridge infrastructure — the ControllerRegistry lazy singleton +
// every cross-cutting helper (path/id/cache/guard/attestation/db-table)
// — moved to ./memory-bridge/bridge-core.ts (W65, P3.4 cut #2). The
// bridge* functions below all build on these; the 3 lifecycle helpers
