/**
 * V3 memory database schema (SQL DDL).
 *
 * Extracted from memory-initializer.ts (W54, P3.3 cut #2). Pure data
 * constant — the long SQL string is consumed by initializeMemoryDatabase
 * to set up the SQLite schema. Lives in its own file so the schema
 * change history reads independently from the init logic.
 */
