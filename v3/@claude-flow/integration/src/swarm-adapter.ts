...
// The agentic-flow pattern types, V3 swarm types, and adapter config were
// extracted into ./swarm-adapter-types.ts during the P3.38 god-file
// decomposition (W159). Import what the class uses + re-export every public
// type so the package index.ts (and any deep importer of './swarm-adapter.js')
// resolves byte-identically. V3TaskDefinition is re-export-only (unused by
// the remaining code); DEFAULT_CONFIG stays module-private to this surface.
import { DEFAULT_CONFIG } from './swarm-adapter-types.js';
import type {
  AgenticFlowTopology,
