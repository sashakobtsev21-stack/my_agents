 ...
 *
 * Features:
 * - Real HNSW indexing (M=16, efConstruction=200) for HNSW-indexed (measured ~1.9x-4.7x) search
...
// The pattern/result/config/metrics types and the guidance tables were
// extracted into ./types.ts during the P3.51 god-file decomposition
// (W172). Import what the implementation uses + re-export the public
// interfaces; the tables stay module-private to this surface.
import {
  AGENT_PATTERNS,
  DEFAULT_CONFIG,
  DOMAIN_GUIDANCE,
...
export class ReasoningBank extends EventEmitter {
  ...
  async searchPatterns(
    ...
    // Try HNSW search first (HNSW-indexed (measured ~1.9x-4.7x))
