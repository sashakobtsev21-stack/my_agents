/**
 * Cognitive Kernel MCP Tools — state & handlers
 *
 * The default logger, the in-memory fallback state (the mutable
 * currentAttentionState / currentCognitiveLoad bindings stay co-located
 * with their reassigning handlers — TS2540-safe), and the five tool
 * handler implementations with their schemas. These were module-private
 * in the original mcp-tools.ts (P3.65, W186) and are NOT re-exported by
 * the barrel — only the five handler functions are imported by it.
 */
...
export async function workingMemoryHandler(
...
export async function attentionControlHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
...
export async function metaMonitorHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
...
export async function cognitiveLoadHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
