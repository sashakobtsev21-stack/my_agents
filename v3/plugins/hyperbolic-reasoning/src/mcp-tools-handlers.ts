/**
 * Hyperbolic Reasoning MCP Tools — shared bridges & handlers
 *
 * The lazy bridge singletons (mutable lets stay with all their
 * consumers), the embedding/taxonomy stores, the default logger, and
 * the five handler implementations. Extracted verbatim from
 * mcp-tools.ts during campaign-2 wave 35 (W241); only the handlers are
 * imported back.
 */
...
export async function embedHierarchyHandler(
...
export async function taxonomicReasonHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
...
export async function hierarchyCompareHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
...
export async function entailmentGraphHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
