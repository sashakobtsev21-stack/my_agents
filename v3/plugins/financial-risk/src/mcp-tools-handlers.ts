/**
 * Financial Risk MCP Tools — auth/rate-limiting & handlers
 *
 * Extracted verbatim from mcp-tools.ts during campaign-2 wave 25
 * (W231); module-private, only the five handlers are imported back.
 */
...
// Default logger (duplicated verbatim from the pre-split header — both
// modules log under the same tag)
const defaultLogger = {
...
export async function portfolioRiskHandler(
  input: Record<string, unknown>,
  context?: ToolContext
...
export async function marketRegimeHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
...
export async function complianceCheckHandler(
  input: Record<string, unknown>,
  context?: ToolContext
): Promise<MCPToolResult> {
  const logger = context?.logger ?? defaultLogger;
