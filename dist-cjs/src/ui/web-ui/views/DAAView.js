    ...
    createDAAInterface() {
        const tabs = [
            ...
            },
            {
                ...
                content: this.createLifecycleTab()
            },
            {
                label: '📡 Communication',
                content: this.createCommunicationTab()
            },
            {
                label: '🤝 Consensus',
                content: this.createConsensusTab()
            },
            {
                label: '🛡️ Fault Tolerance',
                content: this.createFaultToleranceTab()
            },
            {
                label: '⚡ Optimization',
                content: this.createOptimizationTab()
            }
        ];
...
- max_response_time: 100ms"></textarea>
    ...
    async handleCapabilityMatch(params) {
        const matchParams = {
            task_requirements: params.task_requirements || [
                'compute',
                'memory'
            ],
    ...
    displayCapabilityMatches(matches) {
      ...
      <div class="match-card">
        ...
        <div class="match-capabilities">
          ${match.matchedCapabilities.map((cap)=>`<span class="capability-tag">${cap}</span>`).join('')}
        </div>
    ...
    updateStats() {
        ...
        if (resourcesStat) {
            ...
            if (valueEl) {
                const totalUsage = Array.from(this.resources.values()).reduce((sum, res)=>sum + (res.usage || 0), 0);
        ...
        if (commStat) {
            ...
            if (valueEl) {
                const recentComms = this.communications.filter((c)=>Date.now() - c.timestamp < 60000).length;
...
//# sourceMappingURL=DAAView.js.map