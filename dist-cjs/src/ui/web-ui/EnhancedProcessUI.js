import { printSuccess } from '../../cli/utils.js';
...
export class EnhancedProcessUI {
    ...
    async initializeSwarm() {
        ...
        this.logs = [
            ...
            },
            {
                time: new Date(),
                level: 'info',
                message: 'Swarm orchestration ready'
            },
            {
                time: new Date(),
                level: 'success',
                message: 'Neural engine online'
            }
        ];
    }
    async start() {
    ...
    renderEnhancedHeader() {
        const enhancedStatus = this.systemStats.enhancedMode ? colors.green('ENHANCED') : colors.yellow('FALLBACK');
    ...
    renderOverviewView() {
        ...
        const statsLine1 = stats.slice(0, 2).map((stat)=>`${stat.icon} ${stat.color(stat.value)} ${colors.gray(stat.label)}`).join('  |  ');
        const statsLine2 = stats.slice(2, 4).map((stat)=>`${stat.icon} ${stat.color(stat.value)} ${colors.gray(stat.label)}`).join('  |  ');
        ...
        categories.forEach(([id, category], index)=>{
            ...
            const usageInfo = stats && stats.executions > 0 ? colors.dim(` (${stats.executions} uses)`) : colors.dim(' (unused)');
    ...
    renderNeuralView() {
        ...
        const neuralTools = [
            ...
            },
            {
                key: 'P',
                name: 'neural_predict',
                desc: 'Make AI predictions'
            },
            {
                key: 'S',
                name: 'neural_status',
                desc: 'Check neural network status'
            },
            {
                key: 'A',
                name: 'neural_patterns',
                desc: 'Analyze cognitive patterns'
            },
            {
                key: 'L',
                name: 'model_load',
                desc: 'Load pre-trained models'
            },
            {
                key: 'V',
                name: 'model_save',
                desc: 'Save trained models'
            },
            {
                key: 'R',
            ...
            },
            {
                ...
                desc: 'Compress neural models'
            },
            {
                key: 'E',
                name: 'ensemble_create',
                desc: 'Create model ensembles'
            },
            {
                key: 'F',
                name: 'transfer_learn',
            ...
            },
            {
                key: 'X',
                name: 'neural_explain',
                desc: 'AI explainability'
            },
            {
                key: 'W',
                name: 'wasm_optimize',
                desc: 'WASM SIMD optimization'
            },
            {
                key: 'I',
                name: 'inference_run',
                desc: 'Run neural inference'
            }
        ];
    ...
    renderMonitoringView() {
        ...
        const monitoringTools = [
            ...
            },
            {
                key: 'C',
                name: 'cost_analysis',
                desc: 'Cost and resource analysis'
            },
            {
                key: 'R',
                name: 'trend_analysis',
                desc: 'Analyze performance trends'
            },
            {
                key: 'K',
                name: 'benchmark_run',
            ...
            },
            {
                key: 'S',
                name: 'swarm_monitor',
                desc: 'Real-time swarm monitoring'
            },
            {
                key: 'A',
                name: 'agent_metrics',
                desc: 'Agent performance metrics'
            }
        ];
    ...
    renderWorkflowView() {
        ...
        const workflowTools = [
            ...
            },
            {
                ...
                desc: 'Setup automation rules'
            },
            {
                key: 'P',
                name: 'pipeline_create',
                desc: 'Create CI/CD pipelines'
            },
            {
                key: 'S',
                name: 'scheduler_manage',
                desc: 'Manage task scheduling'
            },
            {
                key: 'T',
                name: 'trigger_setup',
                desc: 'Setup event triggers'
            },
            {
                key: 'W',
            ...
            },
            {
                key: 'R',
                name: 'sparc_mode',
                desc: 'Run SPARC development modes'
            },
            {
                key: 'O',
        ...
        if (activeWorkflows.length > 0) {
            activeWorkflows.slice(-3).forEach((workflow)=>{
                const status = workflow.status === 'completed' ? colors.green('✓') : workflow.status === 'in_progress' ? colors.yellow('◐') : colors.gray('○');
        ...
        const cliCommands = [
            {
                key: 'H',
                name: 'hive-mind',
                desc: 'Hive mind orchestration wizard'
            },
            {
                key: 'G',
                name: 'github',
                desc: 'GitHub operations'
            },
            {
                key: 'T',
                name: 'training',
                desc: 'Neural training commands'
            },
            {
                key: 'A',
                name: 'analysis',
                desc: 'Analysis operations'
            },
            {
                key: 'U',
                name: 'automation',
                desc: 'Automation setup'
            },
            {
                key: 'C',
                name: 'coordination',
                desc: 'Swarm coordination'
            },
            {
                key: 'K',
                name: 'hooks',
    ...
    renderStatusView() {
        ...
        const topCategories = Array.from(this.toolStats.entries()).sort((a, b)=>b[1].executions - a[1].executions).slice(0, 3);
    ...
    getToolCategory(toolName) {
        for (const [category, info] of Object.entries(TOOL_CATEGORIES)){
            if (toolName.includes(category) || category === 'neural' && (toolName.includes('neural_') || toolName.includes('model_')) || category === 'memory' && toolName.includes('memory_') || category === 'monitoring' && (toolName.includes('performance_') || toolName.includes('health_')) || category === 'workflow' && (toolName.includes('workflow_') || toolName.includes('automation_')) || category === 'github' && toolName.includes('github_') || category === 'daa' && toolName.includes('daa_') || category === 'system' && (toolName.includes('security_') || toolName.includes('backup_'))) {
                return category;
            }
        }
        return null;
    ...
    async handleEnhancedMemoryInput(input) {
        const actions = {
            s: ()=>this.executeEnhancedTool('memory_usage', {
                    action: 'store'
                }),
            g: ()=>this.executeEnhancedTool('memory_usage', {
                    action: 'retrieve'
                }),
...
//# sourceMappingURL=EnhancedProcessUI.js.map