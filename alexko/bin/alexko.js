#!/usr/bin/env node
// AlexKo CLI - thin wrapper around @claude-flow/cli with alexko branding
...
// Walk up from alexko/bin/ to find @claude-flow/cli in node_modules
...
if (isMCPMode) {
  ...
  // CLI mode: use alexko branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'alexko',
    description: 'AlexKo - AI Agent Orchestration Platform',
  });
  cli.run()
    .then(() => {
