...
>
> - 🎯 **[`docs/CORE-AGENTS.md`](docs/CORE-AGENTS.md)** — **start here.** Curated ~20-agent core for most work + an honest overlap/consolidation map (quality over quantity). Nothing deleted; the full 123 stay available.
...
1. **Prompt standard** — frontmatter `name` (unique, kebab-case) + `description` (trigger form, "Use when …") + `model` (`haiku`/`sonnet`/`opus`); body with **When to use**, **How you work**, **Output contract**, **Coordination** (who it hands off to / its Tier), **Quality bar & anti-drift**, and a **`## Model & cost`** section. Section matching is **alias-aware** (e.g. `## Workflow`/`## Method`/`## How you coordinate` all satisfy "How you work"), so express the intent — exact heading wording isn't required. Enforcement is **hybrid**: the ~25 core/lead agents (see `CORE_AGENTS` in `scripts/check-agents.mjs`) MUST carry every section (missing → **error**); for all other agents a missing section is a **warning** (recommended, not blocking). `## Model & cost` is required for **every** agent.
2. **Run the check** — `node scripts/check-agents.mjs`. It (a) validates every agent's frontmatter/standard (errors for core, warnings for the rest), (b) **regenerates the catalog + report so the new agent is added to the list**, and (c) **re-verifies connectivity** — every `` `agent-name` `` referenced in any prompt must resolve to a real agent (or a real plugin/skill). Exits non-zero on any error.
