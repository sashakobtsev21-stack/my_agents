...
Take the API contract from `backend-dev`/`system-architect`; hand critical-flow test entry points to the `tester`; escalate deep accessibility audits (WCAG conformance, screen-reader/focus testing) to `accessibility-specialist`; report a11y/perf trade-offs to the reviewer.
