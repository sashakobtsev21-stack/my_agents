...
>
> 📚 **Полный разбор всего:** [`docs/FULL-BREAKDOWN.md`](docs/FULL-BREAKDOWN.md) (или [HTML](docs/full-breakdown.html)) — все 41 скиллов, 168 команд по группам, 33 плагина (что каждый привозит), группы MCP-инструментов и пакеты v3.
...
```
▊ AlexKo V3.6  ● user  │  Opus 4.8  │  ⏱ 6m16s  │  ● 34% ctx
────────────────────────────────────────
🤖 Swarm ○ [ 0/15]    ⚪ CVE 0/0    🧪 Tests 74
```

Панель намеренно **лаконичная** — только то, что реально помогает в работе:

**Шапка:** `▊ AlexKo V<версия>` — продукт · `● user` — git-имя · `Opus 4.8` — модель ·
`⏱` — время сессии · `● N% ctx` — **сколько занято окна контекста** (зелёный <70,
жёлтый ≥70, красный ≥90; на ≥90% пора сохраняться/закругляться — самый практичный
сигнал). Стоимость скрыта по умолчанию; вернуть — `RUFLO_STATUSLINE_SHOW_COST=1`.

**Строка статуса:**
- 🤖 **Swarm `[N/15]`** — сколько агентов работает **прямо сейчас** / максимум (`○` нет / `◉` идёт). Видно, занят ли параллельный пайплайн — не дублировать/не перегружать.
- **`CVE n/m`** — безопасность: ⚪ скан не гоняли · 🟢 чисто · 🟡 в работе · 🔴 есть уязвимости (действуй).
- 🧪 **`Tests N`** — сколько тест-файлов в проекте (грубый «покрыто ли тестами?»).

> Нижняя строка `⏵⏵ accept edits …` — это режим самого Claude Code, не часть панели.

Цифры — из реального состояния (git, тесты, swarm, аудит безопасности), поэтому панель — живой барометр, а не баннер. *(Раньше показывались ещё DDD-домены, ADR, векторы, память и «обученность» — убрали как шум: они оживают только при активной работе со свормом и памятью.)*
...
- **Агенты/скиллы/связи сверены:** 123 агент / 41 скиллов / 168 команд / 33 плагина — фронтматтер валиден, дублей нет, **связность чистая** (любая `ссылка` ведёт на реального агента/плагин/скилл). Авто-правило `scripts/check-agents.mjs` + pre-commit + PostToolUse-хук перепроверяют при изменениях.
