...
Внизу в Claude Code RuFlo рисует свою **статус-панель** — это кастомный statusline,
...
```
▊ RuFlo V3.6 ● user │ Opus 4.8 (1M context) │ ⏱ 269m28s │ ● 90% ctx │ $130.71
─────────────────────────────────────────────────────
...
**Шапка:** `▊ RuFlo V<версия>` — продукт+версия · `● user` — git-пользователь (точка
...
красный ≥90) · `$N` — стоимость сессии (можно скрыть `RUFLO_STATUSLINE_HIDE_COST=1`).
...
> Code** (как применяются правки), а не часть RuFlo-панели.
